...
export function GoogleAuthButton({
  ...
  async function handleClick() {
    ...
    if (signupRole) params.set("signup_role", signupRole);
    if (signupCountry) params.set("signup_country", signupCountry);
  ...
  return (
    <div>
      <button
        ...
        onClick={handleClick}
        disabled={pending}
        className="flex w-full items-center justify-center gap-3 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 disabled:opacity-60"
      >
      ...
      {failed && (
        <p className="mt-2 text-center text-xs text-red-600">{errorLabel}</p>
      )}
    </div>
  );
}
